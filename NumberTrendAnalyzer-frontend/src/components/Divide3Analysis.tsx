// src/components/Divide3Analysis.tsx
import React, { useState, useEffect } from 'react';
import './Analysis.css';

const API_BASE_URL = 'http://localhost:8080/api/analysis';

const STORAGE_KEY = 'divide3AnalysisFilters';
...
interface Divide3AnalysisProps {
    onBack?: () => void;
}

const Divide3Analysis: React.FC<Divide3AnalysisProps> = ({ onBack }) => {
    ...
    const fetchAnalysisData = async () => {
        ...
        try {
            ...
            params.append('analysisType', 'divide-3');
    ...
    return (
        <div className="analysis-container">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <h2 className="analysis-title">Phân Tích Kết Quả - Hàm Chia 3</h2>
                {onBack && (
                    <button
                        onClick={onBack}
                        ...
                        <tbody>
                            {data.rows.map((row, index) => (
                                <tr key={index}>
                                    ...
                                    <td className={`match-cell ${row.dauDBMatch === 1 ? 'match' : row.dauDBMatch === 2 ? 'match-2' : 'no-match'}`}>
                                        {row.dauDBMatch}
                                    </td>
                                    <td className="number-cell">{row.db !== null ? String(row.db).padStart(2, '0') : '-'}</td>
                                    <td className={`match-cell ${row.dbMatch === 1 ? 'match' : row.dbMatch === 2 ? 'match-2' : 'no-match'}`}>
                                        {row.dbMatch}
                                    </td>
                                    <td className="number-cell">{row.dauG1 !== null ? String(row.dauG1).padStart(2, '0') : '-'}</td>
                                    <td className={`match-cell ${row.dauG1Match === 1 ? 'match' : row.dauG1Match === 2 ? 'match-2' : 'no-match'}`}>
                                        {row.dauG1Match}
                                    </td>
                                    <td className="number-cell">{row.g1 !== null ? String(row.g1).padStart(2, '0') : '-'}</td>
                                    <td className={`match-cell ${row.g1Match === 1 ? 'match' : row.g1Match === 2 ? 'match-2' : 'no-match'}`}>
                                        {row.g1Match}
                                    </td>
                                </tr>
                            ))}
                        ...
                        <table className="empty-stats-table">
                            ...
                            <tbody>
                                {data.emptyStats.map((stat, index) => {
                                    ...
                                    // Xác định class màu nền dựa trên columnName
                                    const getRowClass = (columnName: string) => {
                                        if (columnName === 'Đầu ĐB') return 'stats-row-dau-db';
                                        if (columnName === 'ĐB') return 'stats-row-db';
                                        if (columnName === 'Đầu G1') return 'stats-row-dau-g1';
                                        if (columnName === 'G1') return 'stats-row-g1';
                                        return '';
                                    };
                                    
                                    return (
                                        <tr key={index} className={getRowClass(stat.columnName)}>
...
export default Divide3Analysis;
